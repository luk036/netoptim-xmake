...
class negCycleFinder {
  ...
  public:
    ...
    auto find_neg_cycle(Container&& dist, WeightFn&& get_weight) -> std::vector<edge_t> {
        this->_pred.clear();
        this->_edge.clear();
  ...
  private:
     ...
     */
    template <typename Container, typename WeightFn>
    auto _is_negative(const node_t& handle, const Container& dist, WeightFn&& get_weight) -> bool {
        auto v = handle;
        do {
            const auto u = this->_pred[v];
            const auto e = this->_edge[v];
            const auto wt = get_weight(e);  // TODO
            if (dist[v] > dist[u] + wt) {
                return true;
            }
            v = u;
